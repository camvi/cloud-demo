...
function LivenessVerify(props) {
            ...
            .then((response) => {
                response.json()
                    .then((data) => {
                        ...
                        if(data.status === 'fail') {
                            //setErrorMessage('Please, follow the prompt')
                            //setHasError(true)
                            
                           // if(timeEnded) {
                                //console.log('time ended - 2')
                           //     setHasError(true)
                           // }
                           // else {
                                setTimeout(() => {
                        ...
                        if(hints[attempt - 1].value === 'el') {
                            ...
                            if (data.face_count_input === 1 && data.faces[0].eye1_open < 0.2 && data.faces[0].eye2_open  < 0.2) {
                                //setHintStatus(true)
                                setAttempt(2)
                                if(attempt === 2) setFaceDetected(true)
                            } else {
                    ...
                    .catch((e) => {
                        // Ignore exception OR exit???. For exit, set hasError to true
    ...
    const handleImage = () => {
        ...
        }, 103) // Not divisible by 5000//5000)
    }
    ...
    useEffect(() => {
        ...
        //console.log("time ended=" + timeEnded + ",hasError=" + hasError)
        if(hasError || timeEnded) {
            props.setVerifyStatus({
                message: 'Verification failed',
                status: 0
            })
        ...
        if (doVerify) {
            //console.log("calling handleverifyimage...")
            props.setLoading(true)
            props.handleVerifyImage(imageSrc)
        }
    }, [hasError, doVerify, timeEnded]);
